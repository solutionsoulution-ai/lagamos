    ...
    <div className="py-24 overflow-hidden">
        ...
        <div className="bg-white py-12 sm:py-16 border-y border-gray-100 shadow-sm">
          <div className="animate-infinite-scroll">
            {extendedPartners.map((partner, i) => (
              <div 
                key={i} 
                className="flex items-center justify-center px-12 sm:px-20 shrink-0 group"
              >
                <span className="text-2xl sm:text-4xl font-bold text-gray-300 tracking-tighter group-hover:text-blue-600 transition-all duration-300 select-none">
                  {partner.name}
                </span>
              </div>
            ))}
      ...
      <div className="mt-16 flex flex-col items-center gap-6 text-center">
        ...
        <p className="text-gray-400 text-[10px] sm:text-xs font-medium italic max-w-3xl px-8 leading-relaxed opacity-80">
          * {language === 'fr' ? 'Europfy collabore en toute transparence avec ces institutions financières pour garantir la sécurité de vos fonds.' : t.p}
        </p>
      </div>
    </div>
  );
