  ...
  return (
    <div className="bg-white rounded-[2.5rem] p-8 lg:p-12 border border-gray-100 shadow-2xl">
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <h3 className="text-3xl font-black text-gray-900 mb-6">{t.h3.split(' ')[0]} <span className="text-blue-600">{t.h3.split(' ').slice(1).join(' ')}</span></h3>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="w-full bg-blue-100 h-4 rounded-full overflow-hidden"><div className="bg-blue-600 h-full w-[35%] animate-pulse"></div></div>
            </div>
          </div>
        </div>
        <div className="bg-blue-50 rounded-3xl p-8 flex flex-col justify-center items-center text-center space-y-4">
          <div className="bg-white p-4 rounded-2xl shadow-sm"><ArrowDownToLine className="w-10 h-10 text-blue-600" /></div>
          <p className="text-4xl font-black text-blue-900">-3.45%</p>
          <p className="text-blue-700 font-semibold italic text-lg">{t.saving}</p>
        </div>
      </div>
    </div>
  );
...
export const StepByStep: React.FC<WidgetProps> = ({ language }) => {
  ...
  const steps = [
    ...
    { icon: FileCheck, title: t.s2t, desc: t.s2d, color: "bg-blue-50 text-blue-600" },
    { icon: Landmark, title: t.s3t, desc: t.s3d, color: "bg-green-50 text-green-600" }
  ];
  ...
  return (
    <div className="bg-gray-50 border-y border-gray-200 py-10">
      <div className="max-w-7xl mx-auto px-4 flex flex-wrap justify-center gap-12 lg:gap-24 grayscale opacity-60">
        <div className="flex items-center gap-3"><ShieldCheck className="w-8 h-8 text-blue-600" /> <span className="font-bold text-gray-900 tracking-tight">{t.rgpd}</span></div>
        <div className="flex items-center gap-3"><Zap className="w-8 h-8 text-blue-600" /> <span className="font-bold text-gray-900 tracking-tight">{t.h24}</span></div>
        <div className="flex items-center gap-3"><Landmark className="w-8 h-8 text-blue-600" /> <span className="font-bold text-gray-900 tracking-tight">{t.orias}</span></div>
      </div>
    </div>
  );
};
