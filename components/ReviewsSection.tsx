...
const ReviewsSection: React.FC<ReviewsSectionProps> = ({ language, customReviews }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (scrollRef.current) {
        ...
        // Si on arrive à la fin (avec une marge de 10px), on revient au début
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (scrollRef.current) {
      ...
      // On calcule dynamiquement la largeur d'un élément basé sur le premier enfant
      const firstChild = scrollRef.current.firstChild as HTMLElement;
      const itemWidth = firstChild ? firstChild.offsetWidth + 24 : clientWidth * 0.8; // 24 est le gap (gap-6)
    ...
    <div className="py-12 relative group/section">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
        <div className="max-w-2xl space-y-4">
          ...
          <h2 className="text-3xl sm:text-5xl font-black text-gray-900 leading-tight">{t.h2}</h2>
        ...
        <div className="flex gap-3">
          <button 
            ...
            className="p-4 rounded-2xl bg-white border border-gray-100 shadow-lg hover:bg-emerald-600 hover:text-white transition-all group active:scale-90"
            aria-label="Previous"
          >
      ...
      <div 
        ...
        className="flex gap-6 overflow-x-auto pb-8 no-scrollbar snap-x snap-mandatory touch-pan-x"
        style={{ scrollBehavior: 'smooth' }}
      >
        {reviews.map((review) => (
          <div 
            key={review.id} 
            className="flex-none w-[85%] sm:w-[450px] snap-center"
          >
            <div className="bg-white p-8 sm:p-12 rounded-[2.5rem] border border-gray-100 shadow-xl relative overflow-hidden group hover:shadow-2xl transition-all h-full flex flex-col hover:-translate-y-1 duration-500">
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <p className="text-gray-600 italic mb-10 flex-grow leading-relaxed text-lg sm:text-xl font-medium">
                  "{review.content}"
                </p>
